...
import pdb
@register_flow("mlp_nc_trainer")
class MLPNodeClassification(BaseFlow):
    r"""
    ...
    def __init__(self, args):
        ...
        super(MLPNodeClassification, self).__init__(args)
    ...
    def preprocess(self):
        ...
        super(MLPNodeClassification, self).preprocess()
    ...
    def _full_train_step(self):
        ...
        h_dict = self.model.input_feature()
        pdb.set_trace()
        logits = self.model(h_dict)[self.category]
    ...
    def _full_test_step(self, modes, logits=None):
        ...
        with torch.no_grad():
            ...
            logits = logits if logits else self.model( h_dict)[self.category]
            masks = {}
            for mode in modes:
                if mode == "train":
